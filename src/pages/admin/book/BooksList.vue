<template>
    <div class="w-full">
        <div class="mb-1 flex justify-between items-center" >
            ...
            <router-link class="px-3 py-1  border-slate-700 border  hover:text-white hover:bg-slate-800" to="create-book">New Book</router-link>
        </div>
                ...
                <tbody>
                    <tr class="hover:bg-gray-50" v-for="book in paginatedBooks" :key="book.id">
                        ...
                        <td class="border border-slate-300 p-1 ">
                            <p class="line-clamp-2 overflow-hidden text-ellipsis break-words max-w-xs text-justify px-5" >{{ book.description }}</p>
                        </td>
                        <td class="border border-slate-300 p-1">
                            <a class="underline" :href="book.cover_image">View</a>
                        </td>
                        <td class="border border-slate-300 p-1">{{ book.isbn }}</td>
                        <td class="border border-slate-300 p-1">
                            <div class="flex flex-wrap gap-1 justify-center">
                                <router-link class="bg-green-900 py-2 px-4 rounded-sm text-sm cursor-pointer text-white hover:bg-green-800" :to="`/edit-book/${book.id}`">Edit</router-link>
                                <button class="bg-red-800 hover:bg-red-700 py-2 px-4 rounded-sm text-sm cursor-pointer text-white" @click="deleteBook(book.id)">Delete</button>
                            </div>
                        </td>
                    </tr>
                </tbody>
